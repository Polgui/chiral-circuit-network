function Fid=Copy_of_state_fidelity(Gamma,Lambda,Lambda_left,Gamma_target,Lambda_target,Lambda_left_target)
...
tens=tensor_contraction(conj(temp),tens,3,1);
tens=tensor_contraction(temp,tens,[3,1],[3,1]);

for iters=3:Ntot-1
    
    temp=Lambda_multiplication(Gamma{iters},Lambda{iters},2);
    tens=tensor_contraction(tens,temp,3,1);
    tens=tensor_contraction(tens,conj(temp),4,1);
    
    temp=Lambda_multiplication(Gamma_target{iters-1},Lambda_target{iters-1},2);
    tens=tensor_contraction(conj(temp),tens,[1,3],[2,3]);
    tens=tensor_contraction(temp,tens,[1,3],[2,3]);
    
end

temp=Lambda_multiplication(Gamma{Ntot},Lambda{Ntot},2);
tens=tensor_contraction(tens,temp,3,1);
tens=tensor_contraction(tens,conj(temp),[4,6],[1,2]);
...
tens=tensor_contraction(tens,temp,[1,2,3,4],[3,1,2,4]);
