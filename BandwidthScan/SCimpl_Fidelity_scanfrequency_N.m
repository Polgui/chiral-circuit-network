...
deltavec=10.^linspace(-2,1,1001);
Fid=zeros(length(Nqubitsvec),length(deltavec));
Fid0=zeros(length(Nqubitsvec),length(deltavec));
Fid1=zeros(length(Nqubitsvec),length(deltavec));
...
%     [U1,V1]=eig(rho1);
    ...
    for iterdelta=1:length(deltavec)
        ...
        delta=deltavec(iterdelta)/(Nqubits);

   %     t0q=-(gamma+1j*(delta-chi))/(gamma-1j*(delta-chi));
   %     t1q=-(gamma+1j*(delta))/(gamma-1j*(delta));
   
        t0q=-(gammar+2*1j*(delta-chi/2))/(gammar-2*1j*(delta-chi/2));
        t1q=-(gammar+2*1j*(delta+chi/2))/(gammar-2*1j*(delta+chi/2));
       ...
       % Fid(iterNqubits,iterdelta)=abs(state_overlap(Gammatemp0,Lambdatemp,GammaZ0,LambdaZ)+state_overlap(Gammatemp1,Lambdatemp,GammaZ1,LambdaZ))^2;
        Fid0(iterNqubits,iterdelta)=state_fidelity_v3(Gammatemp0,Lambdatemp,1,GammaZ0,LambdaZ,1)/sqrt(state_fidelity_v3(GammaZ0,LambdaZ,1,GammaZ0,LambdaZ,1));
        Fid1(iterNqubits,iterdelta)=state_fidelity_v3(Gammatemp1,Lambdatemp,1,GammaZ1,LambdaZ,1)/sqrt(state_fidelity_v3(GammaZ1,LambdaZ,1,GammaZ1,LambdaZ,1));
...
box on
for iterNqubits=1:length(Nqubitsvec)
    h=loglog(deltavec,1-Fid0(iterNqubits,:)-Fid1(iterNqubits,:));
    hold on
    h.LineWidth=3;
...
h=loglog(deltavec,(deltavec.^2),'--r');
h.LineWidth=3;
...
pleg.String{end}='${n}_G^2\delta_p^2/\gamma_r^2$';

ax.XLabel.Interpreter='latex';
ax.XLabel.String='${n}_G|\delta_p|/\gamma_r$';
