...
gammalossvec=10.^linspace(-5,-1,21);

Omegavec=10.^linspace(0,1,20);

overlap=zeros(length(Omegavec),length(gammalossvec));

for itergammaloss=1:length(gammalossvec)
    itergammaloss
   
    for iterOmega=1:length(Omegavec)

        chi12=1e8;
        gammaloss=gammalossvec(itergammaloss);
        ...
        overlap(iterOmega,itergammaloss)=trace(psidimer*psidimer'*rho);
    end
end
...
box on
for iterOmega=1:length(Omegavec)
    h=loglog(Omegavec(iterOmega).^2*gammalossvec/gammar^3,1-overlap(iterOmega,:));
    hold on
    h.LineWidth=6;
...
ax.XLabel.String='$\Omega^2\gamma_\varphi/\gamma_r^3$';
h=loglog([1e-6,1e0],50*[1e-6,1e0],'--r');
h.LineWidth=6;
